...
if __name__=='__main__':
    ...
    parser.add_argument('--config', type=str, default='her2st/Img2Gene', help='logger path.')
    ...
    parser.add_argument("--encoder", type=str, default="res18",choices=["res18", "res50","res101","res152","conch","uni", "gigapath"], help="choose the pre-trained model")
    ...
    if args.encoder == "res18":
        model = load_model_weights("weights/tenpercent_resnet18.ckpt")
        model = model.to(device = device)   
        model.eval()
    elif args.encoder == "res50":
        print("------------------%s-------------"%args.encoder)
        model = get_encoder(args.encoder)  
        _ = model.eval()
        ...
        _ = model.eval()
        model = model.to(device)
    elif args.encoder == "res152":
        print("------------------%s-------------"%args.encoder)
        model = get_encoder(args.encoder)  
        _ = model.eval()
        model = model.to(device)
    elif args.encoder == "conch":
        print("------------------%s-------------"%args.encoder)
        model = get_encoder(args.encoder)  
        _ = model.eval()
        model = model.to(device)
